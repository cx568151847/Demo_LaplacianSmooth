...
void Cloth::setupVBOs() {
  ...
  for (int i = 0; i < nx; i++) {
    for (int j = 0; j < ny; j++) {
      ...
      cloth_velocity_visualization.push_back(VBOPosColor(pos+dt*100*vel,Vec3f(1,1,1)));//红色为速度
	  ...
	  //合外力
	  const Vec3f &acc = p.getAcceleration();
	  cloth_force_visualization.push_back(VBOPosColor(pos, Vec3f(0, 0, 1)));//rgb
	  cloth_force_visualization.push_back(VBOPosColor(pos+dt* acc, Vec3f(0, 0, 1)));//蓝色为加速度
...
void Cloth::cleanupVBOs() { 
  ...
  glDeleteBuffers(1, &cloth_force_visualization_VBO);
