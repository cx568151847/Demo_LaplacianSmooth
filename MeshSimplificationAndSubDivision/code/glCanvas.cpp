...
void GLCanvas::keyboard(unsigned char key, int /*x*/, int /*y*/) {
  ...
  case 's': case 'S':
	...
	//mesh->ModifiedButterflySubdivision();
    mesh->setupVBOs();
    glutPostRedisplay();
    break;
  case 'd': case 'D':
	mesh->Simplification_QEM((int)floor(0.9*mesh->numTriangles()));//使用QEM选择边
    //mesh->Simplification((int)floor(0.9*mesh->numTriangles()));//随机选择边
	 //mesh->Simplification((int)floor(mesh->numTriangles()-1));//随机选择边( one )
    mesh->setupVBOs();
    glutPostRedisplay();
    break;
  case 'q':  case 'Q':
